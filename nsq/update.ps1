...
function global:au_GetLatest {
	...
	(curl -UseBasicParsing "https://api.github.com/repos/nsqio/nsq/releases?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
		...
		if ($url) { $versions[$version] = @{ # append to external $versions variable since powershell does not reduce well, because in powershell it is possible for hashtable to have items with same names
			...
			URL32   = $url
			Version = $_.tag_name -replace '^v',''
			FileFullPath = "`$env:TMP\$(($url -split '/')[-1] -replace '\.gz$')"
		} } }
	}
