...
function global:au_GetLatest {
	...
	(curl -UseBasicParsing "https://api.github.com/repos/LongSoft/UEFITool/releases?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
			[version]$_.tag_name
		}
		catch {
		...
		$url = ($_.assets -match 'UEFITool_.+_win32.zip').browser_download_url
